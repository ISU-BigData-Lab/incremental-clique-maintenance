...
public class SymDiffChangingDensity_deletion {
	...
	// IOException{
	public SymDiffChangingDensity_deletion(Set<int[]> batch) throws IOException {
	...
	*/
	...
	/*
		...
		*/
		...
		//out.println("Number of deleted edges: " + deleted_edge_count);
		//out.println("Current graph density: " + G.density());
		...
		G = new Graph(graph);	//starting with the original graph
		
		int initial_edges = G.numE();
		int num_vertices = G.numV();
		
		double initial_density = (double)2.0*initial_edges/((double)num_vertices*(num_vertices-1));
		
		out.println("initial density: " + initial_density);
		...
		while (true) {
			...
			//FileWriter gw = new FileWriter(graph + "_decremental_" + count, true);
			
			while(edge_count < 100000*count) {
				edge_count++;
			...
			int deleted_edges_initially = edge_count;
			
			int num_edges = initial_edges - edge_count;
			
			initial_density = (double)2.0*num_edges/((double)num_vertices*(num_vertices-1));
			...
			subSize = 0;
			new SymDiffChangingDensity_deletion(batch);
			...
			out.println(count + "," + batch_size + "," + ncliqueCount + "," + subCliqueCount + "," + total_number + "," + ncliqueSizes + "," + subSize + "," + total_size + "," + newCliqueSizeInNodes + "," + subCliqueSizeInNodes + ","
					+ total_size_in_nodes + "," + new_time + "," + sub_time + "," + recompute_time + "," + other_time + "," + cumulative_time  + "," + sc + "," + deleted_edges_initially + "," + initial_density);
			out.close();
